...
import my_functions
import secret_list
from globals_var import current_music, client_bot, tree, my_logger
...
async def on_voice_state_update(member, before, after):
    ...
    if member == client_bot.user and before.channel and after.channel:
        if len(after.channel.members) == 1:
            client_bot.loop.create_task(my_functions.disconnect_bot(voice_client, member.guild.id))
        else:
            voice_client.resume()
        return
...
async def self(interaction: discord.Interaction):
    await client_bot.loop.create_task(my_functions.send_by_interaction(interaction, 'Request received'))
    await client_bot.loop.create_task(queue_gestion.clear_queue(interaction))
    await client_bot.loop.create_task(my_functions.send_by_channel(interaction.channel,
...
async def self(interaction: discord.Interaction, position: int, new_position: int):
    await client_bot.loop.create_task(my_functions.send_by_interaction(interaction, 'Request received'))
    await client_bot.loop.create_task(queue_gestion.move_music(interaction, position, new_position))
...
async def self(interaction: discord.Interaction, music: str, position: int = None):
    await client_bot.loop.create_task(my_functions.send_by_interaction(interaction, 'Request received'))
    await client_bot.loop.create_task(voice_gestion.play(interaction, music, position=position))
...
async def self(interaction: discord.Interaction, shuffle: bool = False, position: int = None):
    ...
    if interaction.guild_id not in secret_list.default_musics:
        await client_bot.loop.create_task(my_functions.send_by_channel(interaction.channel,
                                                                       "No default music for this server"))
        return

    await client_bot.loop.create_task(voice_gestion.play(interaction, secret_list.default_musics[interaction.guild_id],
...
async def self(interaction: discord.Interaction, music: str, shuffle: bool = False):
    await client_bot.loop.create_task(my_functions.send_by_interaction(interaction, 'Request received'))
    await client_bot.loop.create_task(voice_gestion.play(interaction, music, position=1, shuffle=shuffle))
...
async def self(interaction: discord.Interaction, music: str):
    await client_bot.loop.create_task(my_functions.send_by_interaction(interaction, 'Request received'))
    await client_bot.loop.create_task(voice_gestion.play(interaction, music, shuffle=True))


@tree.command(name="queue", description="Display the queue.")
async def self(interaction: discord.Interaction, page: int = 1):
    await client_bot.loop.create_task(my_functions.send_by_interaction(interaction, 'Request received'))
    await client_bot.loop.create_task(queue_gestion.get_queue(interaction, page))
...
async def self(interaction: discord.Interaction, begin: int, end: int = None):
    ...
    await client_bot.loop.create_task(queue_gestion.remove_musics(interaction, begin, end))
...
async def self(interaction: discord.Interaction):
    await client_bot.loop.create_task(my_functions.send_by_interaction(interaction, 'Request received'))
    await client_bot.loop.create_task(queue_gestion.shuffle_queue(interaction))
    await client_bot.loop.create_task(my_functions.send_by_channel(interaction.channel,
