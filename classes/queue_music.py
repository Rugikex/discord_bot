...
class QueueMusic:
    ...
    def get_queue_total_time(self, guild_id: int) -> datetime.timedelta:
        ...
        res += current_music_info.get_music().duration
    ...
    async def remove_musics(self, interaction: discord.Interaction, begin: int, end: int, send_msg: bool = True):
        voice_client = await voice_gestion.check_voice_client(interaction)
        if voice_client is None:
            return
    ...
    async def get_queue(self, interaction: discord.Interaction, page: int, is_new: bool = True):
        ...
        if self.message and not is_new:
            ...
            await self.edit_buttons_on_message_queue(
                self.message, page
            )
        else:
            self.message = await my_functions.send_by_channel(interaction.channel, content)
            await self.create_buttons_on_message_queue(
                self.message, page
            )


    async def add_in_queue(
        self, interaction: discord.Interaction, musics: List[MusicItem], position: int, content: str
    ):
        if position is not None:
            for i in range(len(musics)):
                self.musics.insert(
                    position - 1 + i, musics[i]
                )
            if len(musics) > 1:
                await my_functions.send_by_channel(
                    interaction.channel,
        ...
        # elif self.is_new:  # The first music is the welcome music
    ...
    def message_queue(self, interaction: discord.Interaction, page: int) -> str:
        number_musics = 1
        max_page = max(
            math.ceil(len(self.musics) / 10), 1
        )
    ...
    def create_view_queue(self, page: int) -> discord.ui.View:
        ...
        async def button_callback(interact: discord.Interaction):
            await self.get_queue(interact, page - 1, is_new=False)
    ...
    async def create_buttons_on_message_queue(self, message: discord.Message, page: int):
